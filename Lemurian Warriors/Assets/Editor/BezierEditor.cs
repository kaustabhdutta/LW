...
public class BezierEditor : Editor
{
    ...
    private void ShowDirections()
    {
        ...
        Handles.DrawLine(point, point + curve.GetDirection(0f) * dScale);
        for (int i = 0; i < lineSteps; i++)
        {
            point = curve.GetPoint(i / (float)lineSteps);
            Handles.DrawLine(point, point + curve.GetDirection(i / (float)lineSteps) * dScale);
        }
    }
